...
class machiningListAPIView(APIView):
    pagination_class = LargeResultsSetPagination

    def get(self, request):
        try:
            queryset = machining.objects.all().order_by('-date')

            # 🔍 Filters
            filters = {
                'component': request.query_params.get('component'),
                'machine_no': request.query_params.get('machine_no'),
                'mc_type': request.query_params.get('mc_type'),
            ...
            serializer = MachiningSerializer(paginated_qs, many=True)
...
class machiningFieldsAPIView(APIView):
    def get(self, request):
        ...
        fields_info = [
            ...
            for f in machining._meta.fields
        ]
        return Response(fields_info)
    
class machiningExportAPIView(APIView):
    def get(self, request):
        try:
            queryset = machining.objects.all().order_by('-date')
            ...
            if selected_fields:
                ...
                # default: all fields
                selected_fields = [f.name for f in machining._meta.fields]
