...
class HeatTreatmentListAPIView(APIView):
    pagination_class = LargeResultsSetPagination

    def get(self, request):
        try:
            queryset = HeatTreatment.objects.all().order_by('-date')

            # 🔍 Filters
            filters = {
                'component': request.query_params.get('component'),
                'process': request.query_params.get('customer'),
                'heat_no': request.query_params.get('heat_number'),
                'furnace': request.query_params.get('line'),
            ...
            serializer = HeatTreatmentSerializer(paginated_qs, many=True)
...
class HeatTreatmentFieldsAPIView(APIView):
    def get(self, request):
        ...
        fields_info = [
            ...
            for f in HeatTreatment._meta.fields
        ]
        return Response(fields_info)
    
class HeatTreatmentExportAPIView(APIView):
    def get(self, request):
        try:
            queryset = HeatTreatment.objects.all().order_by('-date')
            ...
            if selected_fields:
                ...
                # default: all fields
                selected_fields = [f.name for f in HeatTreatment._meta.fields]
