...
class VisualListAPIView(APIView):
    pagination_class = LargeResultsSetPagination

    def get(self, request):
        try:
            queryset = Visual.objects.all().order_by('-date')
            ...
            serializer = VisualSerializer(paginated_qs, many=True)
...
class VisualFieldsAPIView(APIView):
    def get(self, request):
        ...
        fields_info = [
            ...
            for f in Visual._meta.fields
        ]
        return Response(fields_info)
    
class VisualExportAPIView(APIView):
    def get(self, request):
        try:
            queryset = Visual.objects.all().order_by('-date')
            ...
            if selected_fields:
                ...
                # default: all fields
                selected_fields = [f.name for f in Visual._meta.fields]
