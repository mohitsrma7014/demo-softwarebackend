...
class MarkingListAPIView(APIView):
    pagination_class = LargeResultsSetPagination

    def get(self, request):
        try:
            queryset = marking.objects.all().order_by('-date')
            ...
            serializer = MarkingSerializer(paginated_qs, many=True)
...
class MarkingFieldsAPIView(APIView):
    def get(self, request):
        ...
        fields_info = [
            ...
            for f in marking._meta.fields
        ]
        return Response(fields_info)
    
class markingExportAPIView(APIView):
    def get(self, request):
        try:
            queryset = marking.objects.all().order_by('-date')
            ...
            if selected_fields:
                ...
                # default: all fields
                selected_fields = [f.name for f in marking._meta.fields]
